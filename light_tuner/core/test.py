...
class Test(multiprocessing.Process):
    ...
    def __init__(
        ...
        # 初始化日志 - 测试实例创建
        logger.info(f"[测试实例-{self.id}] 初始化完成 | 超参数: {self.hparams}")
    ...
    def run(self) -> None:
        ...
        logger.info(f"[测试实例-{self.id}] 开始执行 | PID: {self.pid}")
        ...
        try:
            ...
            if not temp_file_path:
                raise RuntimeError(f"测试[{self.id}]：创建临时文件失败")
            logger.info(f"[测试实例-{self.id}] 临时文件创建成功 | 路径: {temp_file_path}")

            # 步骤3：运行注入后的用户代码
            logger.info(f"[测试实例-{self.id}] 开始执行训练代码 ✨")
            runpy.run_path(path_name=temp_file_path, run_name="__main__")
            logger.info(f"[测试实例-{self.id}] 训练代码执行完成 ✅")

        except Exception as e:
            error_detail = traceback.format_exc()
            logger.error(f"[测试实例-{self.id}] 执行失败 ❌ | 错误: {str(e)}")
            logger.debug(f"[测试实例-{self.id}] 错误堆栈:\n{error_detail}")

        finally:
            # 步骤4：确保临时文件被清理，避免文件残留
            if temp_file_path:
                delete_file(temp_file_path)
                logger.info(f"[测试实例-{self.id}] 临时文件已清理 | 路径: {temp_file_path}")
